...
import { Search, Plus, AlertTriangle, Package, Warehouse, Truck, Store } from "lucide-react";
...
const stockTypeColors = {
  ...
  lorry: 'bg-orange-500/10 text-orange-600',
  store: 'bg-green-500/10 text-green-600',
};

export default function StockByType({ stockType, title }: StockByTypeProps) {
  ...
  const Icon = stockTypeIcons[stockType];
  ...
  const fetchStock = async () => {
    ...
    try {
      ...
      // Fetch stock by size records for specific stock type
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className={`p-3 rounded-xl ${stockTypeColors[stockType]}`}>
            <Icon className="h-6 w-6" />
          </div>
          <div>
            <h1 className="text-3xl font-bold">{title}</h1>
            <p className="text-muted-foreground mt-1">View and manage {title.toLowerCase()} levels</p>
          </div>
        </div>
        <Button onClick={() => {
                    ...
                    <TableRow>
                      <TableCell colSpan={13} className="text-center py-8">
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                          ...
                          <p>No stock found in {title.toLowerCase()}</p>
                          <Button 
                            variant="outline" 
                            size="sm"
                            onClick={() => {
