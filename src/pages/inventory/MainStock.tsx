...
export default function MainStock() {
  ...
  const fetchStock = async () => {
    ...
    try {
      ...
      // Fetch ALL stock (lorry + store) to calculate main stock as combined total
      const { data: stockBySizeData, error: stockError } = await supabase
        .from("stock_by_size")
        .select("*")
        .in("stock_type", ["lorry", "store"]); // Get both lorry and store stock
      
      if (stockError) throw stockError;

      // Group and sum stock by item_id and size (combining lorry + store)
                    ...
                    <TableRow>
                      <TableCell colSpan={12} className="text-center py-8">
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                          ...
                          <p>No stock found</p>
                          <p className="text-sm">Add stock to Lorry Stock or Warehouse Stock</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
