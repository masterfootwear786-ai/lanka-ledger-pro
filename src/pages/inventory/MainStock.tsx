...
export default function MainStock() {
  ...
  const fetchStock = async () => {
    ...
    try {
      ...
      // Fetch stock by size records for BOTH lorry and store (combined = main)
      const { data: stockBySizeData, error: stockError } = await supabase
        .from("stock_by_size")
        .select("*")
        .in("stock_type", ["lorry", "store"]);
      
      if (stockError) throw stockError;

      // Group stock by item_id and aggregate lorry + store
      ...
      stockBySizeData?.forEach(stock => {
        ...
        const currentQty = stockMap.get(stock.item_id)!.get(stock.size) || 0;
        stockMap.get(stock.item_id)!.set(stock.size, currentQty + (stock.quantity || 0));
      });
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Main Stock</h1>
            <p className="text-muted-foreground mt-1">Combined total of Lorry Stock + Store Stock (auto-calculated)</p>
          </div>
        </div>
      </div>
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              <CardTitle>Main Stock Items</CardTitle>
              <Badge variant="outline" className="gap-1 text-muted-foreground">
                Read Only - Combined from Lorry + Store
              </Badge>
              {showLowStockOnly && (
                    ...
                    <TableRow>
                      <TableCell colSpan={12} className="text-center py-8">
                        <div className="flex flex-col items-center gap-2 text-muted-foreground">
                          ...
                          <p>No stock found</p>
                          <p className="text-sm">Add stock to Lorry Stock or Store Stock to see combined totals here</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
