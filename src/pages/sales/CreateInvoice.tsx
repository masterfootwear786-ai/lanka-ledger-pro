    ...
    try {
      ...
      if (documentType === 'order') {
        ...
        if (invoice) {
          ...
          // Restore stock from old invoice lines
        ...
        // Update stock for each line item - deduct quantities (allow negative)
        for (const line of lines) {
          ...
          for (const { size, qty } of sizes) {
            ...
            if (existingStock) {
              // Update existing stock - subtract invoice qty (can go negative)
              const newQuantity = (existingStock.quantity || 0) - qty;
              await supabase
                .from('stock_by_size')
                .update({ quantity: newQuantity, updated_at: new Date().toISOString() })
              ...
              // Create new stock record with negative value
              await supabase
                .from('stock_by_size')
                .insert({
                  company_id: profile.company_id,
                  ...
                  size,
                  quantity: -qty, // Start with negative if no existing stock
                });
            }
          }
        }
